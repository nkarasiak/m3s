...
window.__GRID__ = {
  ...
  resForZoom: function (z) { return Math.max(0, Math.min(20, Math.round(z) - 3)); },
  label: function (p) {
    var km = Math.pow(2, 10 - p);
    return 'p' + p + ' (' + (km >= 1 ? km + ' km' : km * 1000 + ' m') + ')';
  },
  cells: function (p, b) {
    var km = Math.pow(2, 10 - p);
    var sub = km >= 1 ? (km * km) + ' km²' : (km * km * 1e6) + ' m²';
