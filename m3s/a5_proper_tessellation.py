...
class A5ProperGrid(BaseGrid):
    ...
    def _grid_indices_to_center(self, lat_index: int, lon_index: int) -> Tuple[float, float]:
    ...
    def _create_oriented_pentagon(self, center_lat: float, center_lon: float, 
                                lat_index: int, lon_index: int) -> List[Tuple[float, float]]:
        """
    ...
    def get_cell_from_point(self, lat: float, lon: float) -> GridCell:
        ...
        vertices = self._create_oriented_pentagon(center_lat, center_lon, 
                                                lat_index, lon_index)
    ...
    def get_cell_from_identifier(self, identifier: str) -> GridCell:
        ...
        # Create polygon
        vertices = temp_grid._create_oriented_pentagon(center_lat, center_lon,
                                                     lat_index, lon_index)
    ...
    def get_neighbors(self, cell: GridCell) -> List[GridCell]:
        ...
        for d_lat, d_lon in neighbor_offsets:
            ...
            try:
                ...
                neighbor_cell = self.get_cell_from_point(neighbor_center_lat, neighbor_center_lon)
        ...
        for lat_index in range(min_lat_index - 1, max_lat_index + 2):
            for lon_index in range(min_lon_index - 1, max_lon_index + 2):
                try:
                    ...
                    if (center_lat < min_lat - self.grid_spacing or 
                        center_lat > max_lat + self.grid_spacing or
                        center_lon < min_lon - self.grid_spacing or
                        center_lon > max_lon + self.grid_spacing):
                        continue
    ...
    def area_km2(self) -> float:
        ...
        total_cells = (lat_range * cells_per_degree_lat) * (lon_range * cells_per_degree_lon)
    ...
    def _create_pentagon_boundary(self, lat: float, lon: float) -> List[Tuple[float, float]]:
        ...
        lat_index, lon_index = self._quantize_coordinates(lat, lon)
        center_lat, center_lon = self._grid_indices_to_center(lat_index, lon_index)
        vertices = self._create_oriented_pentagon(center_lat, center_lon, lat_index, lon_index)
        return vertices
...
def cell_to_boundary_proper(cell_id: A5Cell, resolution: int) -> List[Tuple[Degrees, Degrees]]:
