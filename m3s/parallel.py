...
try:
    ...
    warnings.warn("Dask not available. Parallel operations will use threading fallback.")
...
try:
    ...
    GPU_AVAILABLE = False
    warnings.warn("RAPIDS/CuPy not available. GPU acceleration disabled.")
...
class ParallelConfig:
    ...
    def __init__(
        ...
        if use_dask and not DASK_AVAILABLE:
            warnings.warn("Dask requested but not available. Using threading fallback.")
        if use_gpu and not GPU_AVAILABLE:
            warnings.warn("GPU requested but RAPIDS/CuPy not available. Using CPU fallback.")
...
class ParallelGridEngine:
    ...
    def _setup_client(self):
        ...
        try:
            ...
            warnings.warn(f"Failed to setup Dask client: {e}. Using threading fallback.")
    ...
    def _intersect_threaded(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_chunk = {
                executor.submit(grid.intersects, chunk): chunk 
                for chunk in chunks
            }
    ...
    def _stream_process_dask(
        ...
        for future in dask_as_completed(futures):
            try:
                ...
                warnings.warn(f"Stream chunk processing failed: {e}")
        ...
        if self.config.use_dask and self._client:
            ...
            for name, future in futures.items():
                try:
                    ...
                    warnings.warn(f"Grid {name} processing failed: {e}")
