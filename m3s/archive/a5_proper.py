...
class A5DodecahedronFaces:
    ...
    def _compute_face_centers(self) -> List[np.ndarray]:
        ...
        # Remaining two faces
        centers.append(np.array([1 / math.sqrt(1 + phi * phi), 0, phi / math.sqrt(1 + phi * phi)]))
        centers.append(np.array([-1 / math.sqrt(1 + phi * phi), 0, -phi / math.sqrt(1 + phi * phi)]))
...
class A5Serialization:
    ...
    def serialize(cell: A5Cell) -> int:
        ...
        if cell.resolution > A5Constants.MAX_RESOLUTION:
            raise ValueError(f"Resolution {cell.resolution} exceeds maximum {A5Constants.MAX_RESOLUTION}")
        ...
        if cell.resolution >= 2:
            ...
            result = (result << (64 - 6 - cell.resolution)) | (cell.s << (64 - 6 - cell.resolution))
...
class A5ProperGrid(BaseGrid):
    ...
    def _create_pentagon_vertices(self, center: np.ndarray, radius: float) -> List[Tuple[float, float]]:
    ...
    def _encode_cell(self, base_cell: int, subdivisions: List[int], lat: float, lon: float) -> int:
        """Encode A5 cell information into an identifier."""
        # Simplified encoding for testing
        cell = A5Cell(origin=base_cell, segment=subdivisions[0] if subdivisions else 0, 
                     s=subdivisions[1] if len(subdivisions) > 1 else 0, resolution=len(subdivisions))
    ...
    def __init__(self, precision: int):
        ...
        if not 0 <= precision <= A5Constants.MAX_RESOLUTION:
            raise ValueError(f"A5 precision must be between 0 and {A5Constants.MAX_RESOLUTION}")
    ...
    def _lon_lat_to_cell_estimate(self, lat: float, lon: float) -> A5Cell:
        ...
        if self.precision >= 2:
            ...
            gamma_normalized = (gamma + math.pi) / A5Constants.TWO_PI  # Normalize to [0,1]
            ...
            gamma_scaled = int(gamma_normalized * precision_scale * 1000) % precision_scale
    ...
    def _create_pentagon_for_cell(self, cell: A5Cell, center_lat: float = None, center_lon: float = None) -> List[Tuple[float, float]]:
        ...
        radius = base_radius / (2.2 ** cell.resolution)  # Slower reduction for better areas
        ...
        if cell.resolution >= 2 and cell.s > 0:
            ...
            s_offset_x = radius * 0.05 * s_factor * math.cos(segment_angle + math.pi / 3)  # Reduced from 0.1
            s_offset_y = radius * 0.05 * s_factor * math.sin(segment_angle + math.pi / 3)
        ...
        if len(vertices) > 1:
            ...
            for v in vertices[:-1]:  # Exclude duplicate last vertex
                if not unique_vertices or (abs(v[0] - unique_vertices[-1][0]) > 0.001 or 
                                         abs(v[1] - unique_vertices[-1][1]) > 0.001):
    ...
    def get_cell_from_point(self, lat: float, lon: float) -> GridCell:
        ...
        if (not polygon.is_valid or polygon.geom_type != 'Polygon' or 
            distance_to_input > max_acceptable_distance):
