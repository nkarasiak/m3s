...
class H3Grid(BaseGrid):
    ...
    def get_neighbors(self, cell: GridCell) -> List[GridCell]:
        ...
        try:
            ...
            neighbor_indices = [idx for idx in neighbor_indices if idx != cell.identifier]
    ...
    def _get_cells_in_bbox_fallback(self, min_lat: float, min_lon: float,
                                   max_lat: float, max_lon: float) -> List[GridCell]:
        """
    ...
    def get_edge_length_km(self) -> float:
        ...
        try:
            ...
            edge_lengths_km = {
                0: 4250.546, 1: 1607.220, 2: 606.596, 3: 228.738,
                4: 86.745, 5: 32.788, 6: 12.393, 7: 4.684,
                8: 1.770, 9: 0.669, 10: 0.253, 11: 0.096,
                12: 0.036, 13: 0.014, 14: 0.005, 15: 0.002
            }
    ...
    def get_hexagon_area_km2(self) -> float:
        ...
        try:
            ...
            # Fallback to hardcoded values if function not available
            areas_km2 = {
                0: 18012898.0, 1: 2562182.0, 2: 365870.0, 3: 52215.0,
                4: 7461.0, 5: 1065.0, 6: 152.0, 7: 21.7,
                8: 3.1, 9: 0.44, 10: 0.063, 11: 0.009,
                12: 0.0013, 13: 0.00019, 14: 0.000027, 15: 0.0000038
            }
    ...
    def get_children(self, cell: GridCell) -> List[GridCell]:
        ...
        try:
            ...
            for child_index in child_indices:
                children.append(GridCell(child_index,
    ...
    def get_parent(self, cell: GridCell) -> GridCell:
        ...
        try:
            ...
            return GridCell(parent_index,
                          self._create_h3_polygon(parent_index),
                          self.precision - 1)
        except Exception:
            return cell
    ...
    def compact_cells(self, cells: List[GridCell]) -> List[GridCell]:
        ...
        try:
            ...
            for h3_index in compacted_indices:
                ...
                compacted_cells.append(GridCell(h3_index,
                                              self._create_h3_polygon(h3_index),
                                              cell_resolution))
            return compacted_cells
        except Exception:
            return cells  # Return original cells if compacting fails

    def uncompact_cells(self, cells: List[GridCell], target_resolution: int) -> List[GridCell]:
        """
        ...
        try:
            ...
            for h3_index in uncompacted_indices:
                uncompacted_cells.append(GridCell(h3_index,
                                                self._create_h3_polygon(h3_index),
                                                target_resolution))
    ...
    def intersects(self, gdf: gpd.GeoDataFrame, target_crs: str = "EPSG:4326") -> gpd.GeoDataFrame:
        ...
        """
        if gdf.empty:
            empty_columns = ['cell_id', 'precision', 'utm', 'geometry'] + [col for col in gdf.columns if col != 'geometry']
