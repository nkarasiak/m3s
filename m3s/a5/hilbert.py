"""
Hilbert curve utilities for A5 pentagonal grid system.
...
def ij_to_kj(ij: IJ) -> KJ:
    """Convert IJ to KJ coordinates."""
    i, j = ij
    return (i + j, j)


def kj_to_ij(kj: KJ) -> IJ:
    """Convert KJ to IJ coordinates."""
    k, j = kj
    return (k - j, j)
...
def quaternary_to_flips(n: Quaternary) -> Tuple[Flip, Flip]:
    """Convert a quaternary digit to flip configuration."""
...
def _s_to_anchor(
    s: int, resolution: int, invert_j: bool, flip_ij: bool
) -> Anchor:
    """Internal conversion from s-value to anchor."""
    offset = [0.0, 0.0]
    flips = [NO, NO]
...
def get_required_digits(offset: Tuple[float, float]) -> int:
    """Return number of digits needed to represent an offset."""
...
def ij_to_quaternary(ij: IJ, flips: Tuple[Flip, Flip]) -> Quaternary:
    """Convert IJ coordinates to a quaternary digit with flips."""
    u, v = ij
    digit = 0
...
def _ij_to_s(
    input_ij: IJ, invert_j: bool, flip_ij: bool, resolution: int
) -> int:
    """Internal conversion from IJ coordinates to s-value."""
    ...
    for i in range(num_digits - 1, -1, -1):
        ...
        upscaled_child_offset = (
            child_offset[0] * scale,
            child_offset[1] * scale,
        )
        pivot = [
