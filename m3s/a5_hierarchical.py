...
class A5HierarchicalGrid(BaseGrid):
    ...
    def _create_dodecahedron_base_cells(self) -> List[Tuple[int, List[Tuple[float, float]]]]:
        """
    ...
    def _subdivide_pentagon(self, vertices: List[Tuple[float, float]], subdivision_level: int) -> List[List[Tuple[float, float]]]:
        """
        ...
        subdivision_factor = 1.0 / (2.5 ** subdivision_level)  # Smaller cells at higher levels
    ...
    def _create_pentagon_at_center(self, center_lat: float, center_lon: float, radius: float) -> List[Tuple[float, float]]:
    ...
    def _get_cell_by_hierarchical_id(self, hierarchical_id: int) -> GridCell:
        ...
        for level in range(self.precision):
            ...
            child_index = (path >> (level * 3)) & 0x7  # 3 bits per level = 8 possible children
    ...
    def get_cell_from_point(self, lat: float, lon: float) -> GridCell:
        ...
        if self.precision == 0:
            ...
            polygon = Polygon(vertices) if vertices else Polygon([(0, 0), (1, 0), (1, 1), (0, 0)])
        ...
        coord_hash = hash((round(lat * 10**self.precision), round(lon * 10**self.precision)))
...
def cell_to_lonlat_hierarchical(cell_id: A5Cell, resolution: int) -> Tuple[Degrees, Degrees]:
...
def cell_to_boundary_hierarchical(cell_id: A5Cell, resolution: int) -> List[Tuple[Degrees, Degrees]]:
