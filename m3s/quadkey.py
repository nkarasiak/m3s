...
class QuadkeyGrid(BaseGrid):
    ...
    def _tile_xy_to_lat_lon_bounds(self, tile_x: int, tile_y: int) -> Tuple[float, float, float, float]:
        """
    ...
    def _create_tile_polygon(self, tile_x: int, tile_y: int) -> Polygon:
        ...
        """
        min_lat, min_lon, max_lat, max_lon = self._tile_xy_to_lat_lon_bounds(tile_x, tile_y)
    ...
    def get_cell_from_identifier(self, identifier: str) -> GridCell:
        ...
        if len(identifier) != self.level:
            raise ValueError(f"Quadkey length {len(identifier)} does not match grid level {self.level}")
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                if 0 <= neighbor_x <= max_tile and 0 <= neighbor_y <= max_tile:
                    ...
                    neighbors.append(GridCell(neighbor_quadkey, neighbor_polygon, self.level))
    ...
    def get_children(self, cell: GridCell) -> List[GridCell]:
        ...
        for digit in ['0', '1', '2', '3']:
            ...
            child_polygon = self._create_tile_polygon_for_level(child_tile_x, child_tile_y, self.level + 1)
            children.append(GridCell(child_quadkey, child_polygon, self.level + 1))
        
        return children
    
    def _create_tile_polygon_for_level(self, tile_x: int, tile_y: int, level: int) -> Polygon:
        """
