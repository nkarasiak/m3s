...
class A5FixedGrid(BaseGrid):
    ...
    def _project_to_face_plane(self, xyz: np.ndarray, face_id: int) -> Tuple[float, float]:
    ...
    def _quantize_coordinates(self, lat: float, lon: float) -> Tuple[float, float]:
        ...
        cell_size = base_cell_size / (3.0 ** self.precision)  # Use 3^precision for finer subdivision
    ...
    def _get_cell_data_from_point(self, lat: float, lon: float) -> A5CellData:
        ...
        if self.precision > 0:
            ...
            coord_hash = (lat_int * 360000 + lon_int) % (2 ** min(20, self.precision * 2))
    ...
    def _get_cell_center(self, cell_data: A5CellData, input_lat: float = None, input_lon: float = None) -> Tuple[float, float]:
    ...
    def _create_pentagon(self, center_lat: float, center_lon: float, cell_data: A5CellData) -> List[Tuple[float, float]]:
        ...
        if abs(center_lat) > 85.0:
            ...
            if abs(center_lat) > 89.0:
                ...
                vertices = [
                    ...
                    (center_lon - half_size, min(89.9, center_lat + half_size)),
                    (center_lon - half_size, max(-89.9, center_lat - half_size))  # Close polygon
                ]
        else:
            ...
            for i in range(5):
                ...
                vertex_lon = center_lon + pentagon_size * math.cos(angle) / lat_correction
    ...
    def _create_pentagon_vertices(self, center: np.ndarray, radius: float) -> List[Tuple[float, float]]:
        ...
        cell_data = A5CellData(face=0, segment=0, subdivision=0, resolution=self.precision)
